...
describe('Identity tests users API tests', () => {
  ...
  describe('5. POST /api/keycloak-login', () => {
    it('201 positive test', () => {
      cy.keycloakAPI(cloakUser1)
        .then(resp => {
          ...
          cy.getUserIdAPI(cloakUser1.username, resp.body)
            .then(resp2 => {
              Cypress.env(`id_${cloakUser1.username}`, resp2.body.userId);
              console.log(resp2.body.userId);
              expect(resp2.status).to.equal(200);
            });
        });
    ...
    it('201 negative test: should not create a user if we dont type correctly the parameter identity.', () => {
      ...
      cy.request({
        ...
        body: {
          description: '',
          emaila: `${fakeCloakUser1.username}@hu-berlin.com`,
          firstPameas: `${fakeCloakUser1.username}`,
          identity9: `${fakeCloakUser1.username}`,
        ...
        Cypress.env(`token_${fakeCloakUser1.username}`, resp.body);
        expect(resp.status).to.equal(201);
        cy.getUserIdAPI(fakeCloakUser1.username, resp.body)
          .then(resp2 => {
            Cypress.env(`id_${fakeCloakUser1.username}`, resp2.body.userId);
            console.log(resp2.body.userId);
            expect(resp2.status).to.equal(200);
          });
      });
      // Check does there is no new user
      cy.pause();
    });

    it('500 negative test: should not create a user if we dont type correctly the parameter name.', () => {
      cy.request({
        method: 'POST',
        url: '/api/keycloak-login',
        headers: {
        ...
        body: {
          ...
          issuer: 'https://www.iqb-login.de/realms/iqb',
          lasName: `${fakeCloakUser2.username}`,
          namea: `${fakeCloakUser2.username}`,
          password: ''
        },
    ...
    it('201 positive test: should create a user typing correctly the parameter but identity and name.', () => {
      ...
      cy.request({
        ...
        body: {
          ...
          name: `${fakeCloakUser2.username}`,
          password: ''
        }
      }).then(resp => {
        Cypress.env(`token_${fakeCloakUser2.username}`, resp.body);
        expect(resp.status).to.equal(201);
        cy.getUserIdAPI(fakeCloakUser2.username, resp.body)
          .then(resp2 => {
            Cypress.env(`id_${fakeCloakUser2.username}`, resp2.body.userId);
            expect(resp2.status).to.equal(200);
          });
      });
    });
