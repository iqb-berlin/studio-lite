...
import { buildQueryParameters, buildQueryParametersComplex } from './api';
...
// 12
Cypress.Commands.add('getWsGroupsAPI', (token:string) => {
  const authorization = `bearer ${token}`;
  cy.request({
    method: 'GET',
    url: '/api/admin/workspace-groups',
...
// 83
Cypress.Commands.add('updateWsByUserAPI', (userId:string, groupId: string,
                                           levels: number[], wsIds:string[], token:string) => {
  const authorization = `bearer ${token}`;
  cy.request({
    method: 'PATCH',
    url: `/api/group-admin/users/${userId}/workspaces`,
