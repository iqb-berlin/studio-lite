...
describe('CommentWriteGuard', () => {
  ...
  it('should throw ForbiddenException for a comment of another user', async () => {
    unitCommentService.findOneComment.mockResolvedValue({ id: 42, userId: 2 } as UnitCommentDto);

    await expect(guard.canActivate(contextFor(1, { id: '42' }))).rejects.toThrow(ForbiddenException);
  });
  ...
  it('should throw ForbiddenException for a review session, which owns no comment', async () => {
    await expect(guard.canActivate(contextFor(0, { id: '42' }))).rejects.toThrow(ForbiddenException);
  ...
  it('should throw ForbiddenException without a comment in the route', async () => {
    await expect(guard.canActivate(contextFor(1, {}))).rejects.toThrow(ForbiddenException);
  });
});
