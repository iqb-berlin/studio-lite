...
run_update_script_in_selected_version() {
  ...
  current_update_script=$PWD/backup/release/"$SOURCE_TAG"/update_$APP_NAME.sh
  ...
  if [ ! -f "$current_update_script" ] ||
    ...
    if wget -q -O "$PWD"/update_$APP_NAME.sh "$new_update_script"; then
      chmod +x "$PWD"/update_$APP_NAME.sh
    ...
    bash "$PWD"/update_$APP_NAME.sh "$TARGET_TAG"
    exit $?
