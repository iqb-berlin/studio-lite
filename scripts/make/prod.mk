.PHONY: production-status production-logs production-config production-ramp-up production-shut-down production-start production-stop production-clean
...
production-start: ## Start docker containers
	docker-compose -f $(BASE_DIR)/docker-compose.yml -f $(BASE_DIR)/docker-compose.prod.yml start

production-stop: ## Stop docker containers
	docker-compose -f $(BASE_DIR)/docker-compose.yml -f $(BASE_DIR)/docker-compose.prod.yml stop

production-status: ## Get status list of all containers
	docker-compose -f $(BASE_DIR)/docker-compose.yml -f $(BASE_DIR)/docker-compose.prod.yml ps -a

production-logs: ## Get logs of running containers
	docker-compose -f $(BASE_DIR)/docker-compose.yml -f $(BASE_DIR)/docker-compose.prod.yml logs -f
