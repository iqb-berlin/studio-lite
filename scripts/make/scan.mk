.PHONY: scan-app scan-db scan-backend scan-frontend
...
scan-db:	## scans db image for security vulnerabilities
	cd $(BASE_DIR) && docker build --pull -f $(BASE_DIR)/database/Dockerfile --target=prod --no-cache --rm -t iqbberlin/studio-lite-db:latest -t scm.cms.hu-berlin.de:4567/iqb/studio-lite/iqbberlin/studio-lite-db:latest .
