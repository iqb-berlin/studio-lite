...
dev-db-update-status: .EXPORT_ALL_VARIABLES
	cd $(BASE_DIR) && docker compose run --rm liquibase liquibase --changelogFile=studio-lite.changelog-root.xml \
...
dev-db-generate-docs: .EXPORT_ALL_VARIABLES
	...
	docker compose run --rm liquibase liquibase --changelogFile=studio-lite.changelog-root.xml \
		--url=jdbc:postgresql://db:5432/$(POSTGRES_DB) --username=$(POSTGRES_USER) --password=$(POSTGRES_PASSWORD) \
