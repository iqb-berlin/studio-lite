...
test-app: test-backend test-frontend ## Run all tests (only in combination with 'make dev-up')

test-backend: ## Run backend tests (only in combination with 'make dev-up')
	docker compose --env-file $(BASE_DIR)/.env.dev exec -it backend bash -c "nx test api"

test-frontend: ## Run frontend tests (only in combination with 'make dev-up')
...
test-e2e-api-dev: ## Run all e2e api tests in dev environment (only in combination with 'make dev-up')
	docker compose --env-file $(BASE_DIR)/.env.dev -f $(BASE_DIR)/docker-compose.e2e.yml run --rm test-e2e-api

test-e2e-ui-chrome-dev: ## Run all e2e ui tests with chrome browser in dev environment (only in combination with 'make dev-up')
	docker compose --env-file $(BASE_DIR)/.env.dev -f $(BASE_DIR)/docker-compose.e2e.yml run --rm test-e2e-ui-chrome

test-e2e-ui-chrome-mobile-dev: ## Run all e2e ui tests with chrome browser for mobiles in dev environment (only in combination with 'make dev-up')
	docker compose --env-file $(BASE_DIR)/.env.dev -f $(BASE_DIR)/docker-compose.e2e.yml run --rm test-e2e-ui-chrome-mobile

test-e2e-ui-firefox-dev: ## Run all e2e ui tests with firefox browser in dev environment (only in combination with 'make dev-up')
	docker compose --env-file $(BASE_DIR)/.env.dev -f $(BASE_DIR)/docker-compose.e2e.yml run --rm test-e2e-ui-firefox

test-e2e-ui-firefox-mobile-dev: ## Run all e2e ui tests with firefox browser for mobiles in dev environment (only in combination with 'make dev-up')
	docker compose --env-file $(BASE_DIR)/.env.dev -f $(BASE_DIR)/docker-compose.e2e.yml run --rm test-e2e-ui-firefox-mobile

test-e2e-ui-edge-dev: ## Run all e2e ui tests with edge browser in dev environment (only in combination with 'make dev-up')
	docker compose --env-file $(BASE_DIR)/.env.dev -f $(BASE_DIR)/docker-compose.e2e.yml run --rm test-e2e-ui-edge

test-e2e-ui-edge-mobile-dev: ## Run all e2e ui tests with edge browser for mobiles in dev environment (only in combination with 'make dev-up')
